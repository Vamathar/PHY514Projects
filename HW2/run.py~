...
def source(x,y):
    expx=(x-0.5)*(x-0.5)
    expy=(y-0.5)*(y-0.5)
    return math.exp(-16.*(expx+expy))
#    return math.cos(((x-0.5)**2+(y-0.5)**2)/(((x-0.5)*(y-0.5))**2+1e-4))
#    return 0.

# y=0 and y=1 boundary values are the same problems 1 & 3
def ybound(x):
#    return 1.-math.sin(x/(10*math.pi))
    return 1.-x
#    return 1.
...
################################################################################
    ...
    x,y,z = SolveMe.Grids()
    Plotter = dp(title)
    Plotter.Contour(x,y,z,30,'magma','Problem1Part1Plot.png')
#    steps = 10
...
#    x,y,z = SolveMe.Grids()
#    Plotter = dp(title)
#    Plotter.Contour(x,y,z,30,'plasma','LaplacePlot.png')
#
#
#
#################################################################################
